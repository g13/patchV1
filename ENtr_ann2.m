% [mu,stats] = ENtr_ann2(T,S[,Pi,mu,Kin,alpha,beta,annrate,
...
function [mu,stats] = ENtr_ann2(T,S,Pi,mu,Kin,alpha,beta,annrate,...
...
if ~exist('Kin','var') || isempty(Kin), Kin = 0.5; end;
if ~exist('alpha','var') || isempty(alpha), alpha = ones(N,1); end;
if all(size(alpha) == 1), alpha = repmat(alpha,N,1); end;
if ~exist('beta','var') || isempty(beta), beta = 10; end;
if ~exist('annrate','var') || isempty(annrate), annrate = 0.8; end;
if ~exist('max_it','var') || isempty(max_it), max_it = 20; end;
if ~exist('max_cyc','var') || isempty(max_cyc), max_cyc = 20; end;
if ~exist('min_K','var') || isempty(min_K), min_K = 0.000001; end;
% Don't use "tol" with annealing
% $$$ if ~exist('tol','var') | isempty(tol) tol = 0.0001; end;
if ~exist('tol','var') || isempty(tol), tol = -1; end;
if ~exist('method','var') || isempty(method), method = 'Cholesky'; end;
...
while code<0
    ...
    if K < min_K			% Min. value of K reached, we make K -> 0
        ...
        if ~isempty(E), E_old = E(end,1); end
