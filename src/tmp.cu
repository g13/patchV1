...
int main(int argc, char **argv) {
	...
	eofLGN += sizeof(Float) * 2 * nLGN + sizeof(InputType_t) * nLGN;
	assert(eofLGN == fLGN.tellg);
	size_t true_eof = fLGN.seekg(0, ios::end);
	if (eofLGN == fLGN.tell || useNewSetup) { // Setup LGN here 
		fLGN.close();
    	...
    	norm = normal_distribution<Float>(0.0, 1.0/3.0/sqrt(2)); // within a 1/3 of the std of the gaussian
    	...
    	for (unsigned int i=0; i<nLGN; i++) {
    	    ...
    	    switch (LGNtype[i]) {
    	        ...
    	        case InputType::MonLoff: case InputType::LonMoff: {
    	            ...
    	            // ratio
    	            ratio[i] = get_rand_from_norm(ratio_onC, rGen_LGNsetup, positiveBound);
    	            ratio[i+nLGN] = get_rand_from_norm(ratio_offS, rGen_LGNsetup, positiveBound);
    	            // delay
    	            delay[i] = get_rand_from_norm(delay_onC, rGen_LGNsetup, positiveBound);
    	            delay[i+nLGN] = get_rand_from_norm(delay_offS, rGen_LGNsetup, positiveBound);
    	            break;
    	        }
    	        // off-centers
    	        case InputType::MoffLon: case InputType::LoffMon: {
    	            // k
    				tie(LGN_k[i], LGN_k[i+nLGN]) = get_rands_from_correlated_norm(K_offC, K_onS, rho_Kc_Ks, rho_Kc_Ks_comp, rGen_LGNsetup, rGen_LGNsetup, positiveBound, positiveBound);
					LGN_k[i] *= -1; //off-center

    	            // tauR, nR
    	            tie(tauR[i],nR[i]) = get_rands_from_correlated_norm(tauR_offC, nR_offC, rho_tau_n, rho_tau_n_comp, rGen_LGNsetup, rGen_LGNsetup, positiveBound, positiveBound);
    	            tie(tauR[i+nLGN],nR[i+nLGN]) = get_rands_from_correlated_norm(tauR_onS, nR_onS, rho_tau_n, rho_tau_n_comp, rGen_LGNsetup, rGen_LGNsetup, positiveBound, positiveBound);
    	            // tauD, nD
    	            tie(tauD[i],nD[i]) = get_rands_from_correlated_norm(tauD_offC, nD_offC, rho_tau_n, rho_tau_n_comp, rGen_LGNsetup, rGen_LGNsetup, positiveBound, positiveBound);
    	            tie(tauD[i+nLGN],nD[i+nLGN]) = get_rands_from_correlated_norm(tauD_onS, nD_onS, rho_tau_n, rho_tau_n_comp, rGen_LGNsetup, rGen_LGNsetup, positiveBound, positiveBound);

    	            // ratio
    	            ratio[i] = get_rand_from_norm(ratio_offC, rGen_LGNsetup, positiveBound);
    	            ratio[i+nLGN] = get_rand_from_norm(ratio_onS, rGen_LGNsetup, positiveBound);
    	            // delay
    	            delay[i] = get_rand_from_norm(delay_offC, rGen_LGNsetup, positiveBound);
    	            delay[i+nLGN] = get_rand_from_norm(delay_onS, rGen_LGNsetup, positiveBound);
    	            break;
    	        }
    	...
    	// write to the end of the LGN_file after LGN_x and LGN_y positions
		fLGN.seekp(eofLGN);
    ...
    for (unsigned int it = 0; it < nt; it++) {
        ...
        LGN_nonlinear<<<nLGN_x, nLGN_y>>>(dLGN_fr, dLGN.logistic, max_convol);
