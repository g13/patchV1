...
Float mapping_rule(const Float ecc, const Float normalRand, RandomEngine &rGen, Float LGN_V1_RFratio, PosInt iLayer); // inputLayer

// map ecc to radius
__device__
__forceinline__
Float mapping_rule_CUDA(Float ecc, curandStateMRG32k3a rGen, Float LGN_V1_RFratio, PosInt iLayer) {
	...
	//const Float a*2 = 13.32f;
    const Float a[2] = {5, 10};
	const Float b[2] = {0.037f, 0.074};
	Float mean = a[iLayer] + b[iLayer] * ecc;
	const Float std = 0.01; //3.0; // it is NOT the scatter in the paper, which means the VF center's scatter around an electrode
	const Float lower_bound = 1.0; // TODO: look for the min(LGN RF size, V1 neuron RF size)
    // R = sqrt(area)
