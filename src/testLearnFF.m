function testLearnFF(res_suffix, isuffix, osuffix, res_fdr, data_fdr, fig_fdr, iV1, nt_)
	pLGN = [33, 32, 7, 8]
	if nargin < 7
	...
	fLGN_vpos = [res_fdr, 'LGN_vpos', res_suffix, '.bin']
	%%
	...
	if ~exist('iV1', 'var')
	    ...
	    for i = 1:nt_
	        if rawData 
	            ...
	            fseek(rid, ((iModel+2+(1+haveH)*(ngTypeFF + ngTypeE + ngTypeI))*nV1+nI)*4, 0); % skip
	        else
			...
			tmp = zeros(nV1,1);
			tmp(V1_sInfo >= 1) = V1_sInfo(V1_sInfo >= 1);
	        nsp = nsp + floor(tmp);
	    end
	    if rawData
	        fclose(rid);
	        rid = fopen(rawDataFn, 'r'); % reopen
	        fseek(rid, 9*4, 0); % skip
	    else
	        fclose(fid);
	...
	for i = 1:nt_
	    ...
	    if rawData 
			...
			depC = fread(rid, nV1, 'float');
			if iModel == 1
				w = fread(rid, nV1, 'float');
			end
	        ...
	        fstatus = fseek(rid, (((1+haveH)*(ngTypeFF + ngTypeE + ngTypeI) - 1)*nV1 + nI)*4, 0); % skip
	        if fstatus == -1
	            disp(ferror(rid));
	            break;
	        end
