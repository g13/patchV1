...
struct shared_spat {
    Float xhspan, yhspan, dx, dy, cx, cy;
...
void store_spatialWeight(
    ...
    Float sample_vol = ss.dx * ss.dy;

    SW_storage[storeID] = spatialKernel(x, y, ss.rx, ss.ry);
    Float x_plane, y_plane;
    retina_to_plane(ss.cx + x, ss.cy + y, x_plane, y_plane);
    SC_storage[xID] = x_plane;
    SC_storage[yID] = y_plane;
    dxdy_storage[offset0] = dxdy;
}
...
void store_temporalWeight(
    ...
    for (SmallSize iPatch = 0; iPatch < nPatch; iPatch++) {
        ...
        Size storeID = (id*nType + iType)*nKernelSample + iPatch*patchSize + threadIdx.x;

        Float t = twid*kernelSampleDt + kernelSampleT0
        TW_storage[storeID] = temporalKernel(t, temp, fac1, fac2);
    }
...
void store_weight(
    ...
    __shared__ Float spat[6]; // xhspan, yhspan, dx, dy, cx, cy
    Size id = blockIdx.x;
    SmallSize iType = blockIdx.y;
    Size lid = iType*gridDim.x + id;
    SmallSize nType = gridDim.y;

    store_temporalWeight(temporal, TW_storage, nKernelSample, kernelSampleDt, kernelSampleT0, id, lid, iType, nType);
    ...
    if (storeSpatial) {
        ...
        if (use_shared) { // TODO: compare with global broadcast
            if (threadIdx.y == 0 && threadIdx.x == 0) {
                ...
                spat[0] = xhspan; spat[1] = yhspan; spat[2] = dx; spat[3] = dy; spat[4] = spat0.x; spat[5] = spat0.y;
            }
            __syncthreads();
            // load from shared mem
            shared_spat ss(spat);
            store_spatialWeight(ss, SW_storage, SC_storage, dxdy_storage, nsig, id, iType, nType);
        } 
    }
}
...
void sub_convol(
    ...
    */
    ...
    Float lastDecayIn = decayIn[offset]; // = sum_i2n(F_i[exp(-t_{i+1}/ave_tau) - exp(-t_{i}/ave_tau)] - F_{n+1})*exp(-(t-tau)/ave_tau)
    ...
    for (SmallSize iPatch=0; iPatch<nPatch+1; iPatch++) {
        ...
        if (iPatch == 0 && tid == 0) { // dt < kernelSampleDt, must be in the first patch
            ...
            if (tf0 < tPerFrame) {
                ...
                decayIn[offset] = lastDecayIn*expp(-dt/ave_tau);
            } else {
                // here we ASSUME tPerframe > dt, i.e., at most one change of frame happen within a single dt
                Float F_2 = nSampleShared[0];
                decayIn[offset] = lastDecayIn*expp(-dt/ave_tau) + (F_1 - F_2) * expp(-(tf0- tPerFrame)/ave_tau);
                lastF[id] = F_2;
            }
        }
        ...
        if (tid < nActive) {
            ...
            lastDecayIn *= expp(-t/ave_tau); // sum_i2n(F_i[exp(-t_{i+1}/ave_tau) - exp(-t_{i}/ave_tau)] - F_{n+1})*exp(-(t0-tau)/ave_tau) decayed to t = t0 + t
            for (SmallSize iFrame = 0; iFrame < local_nFrame; iFrame++) {
                ...
                tf0 -= tPerFrame;
                Float decay = expp(-tf0/ave_tau);
                lastDecayIn += (F_1 - F_2) * decay; // F_{n+1} decayed with exp(-((t+t0) - t_{i+2})/ave_tau);
                F_1 = F_2;
            }
        ...
        for (SmallSize iSample = 0; iSample < patchSize; iSample++) {
...
void LGN_convol_c1s(
    ...
    SmallSize nSample = blockDim.x*blockDim.y;
    __shared__ Float nSampleShared[nSample];
    ...
    //TODO: ave_tau may take different value for different cone type
    // convolve center and update decayIn, lastF
    sub_convol(type, nsig, framePhase, ave_tau, kernelSampleDt, nKernelSample, convol, spatial, decayIn, lastF, reduced, nSampleShared, SW_storage, SC_storage, dxdy_storage, TW_storage, lid, tid, 0, 2, dt, spatialStored);

    type = coneType[gridDim.x + id];

    // convolve surround and add to convol and update decayIn, lastF
    sub_convol(type, nsig, framePhase, ave_tau, kernelSampleDt, nKernelSample, convol, spatial, decayIn, lastF, reduced, nSampleShared, SW_storage, SC_storage, dxdy_storage, TW_storage, lid, tid, 1, 2, dt, spatialStored);
